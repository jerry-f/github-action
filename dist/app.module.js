...
import { ConfigModule, mergeConfigOverride, } from './fundamentals/config';
...
function filterOptionalModule(config, module) {
    ...
    if (requirements?.length) {
        ...
        if (nonMetRequirements.length) {
            ...
            new Logger(name).warn(`${name} is not enabled because of the required configuration is not satisfied.`, 'Unsatisfied configuration:', ...nonMetRequirements.map(config => `  AFFiNE.${config}`));
            return null;
        }
    }
...
function buildAppModule() {
    ...
    factor
        ...
        // graphql server only
        .useIf(config => config.flavor.graphql, ServerConfigModule, GqlModule, StorageModule, UserModule, WorkspaceModule, FeatureModule, QuotaModule)
        // self hosted server only
        .useIf(config => config.isSelfhosted, ServeStaticModule.forRoot({

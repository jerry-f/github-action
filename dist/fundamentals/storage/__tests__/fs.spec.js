...
function createProvider() {
    return new FsStorageProvider(config, 'test' + Math.random().toString(16).substring(2, 8));
}
function keys(list) {
    return list.map(i => i.key);
}
async function randomPut(provider, prefix = '') {
...
test('list - one level', async (t) => {
    const provider = createProvider();
    const list = await Promise.all(Array.from({ length: 100 }).map(() => randomPut(provider)));
    list.sort();
...
test('list recursively', async (t) => {
    ...
    await Promise.all([
        ...
        Promise.all(Array.from({ length: 10 }).map(() => randomPut(provider, 'a/'))),
        Promise.all(Array.from({ length: 10 }).map(() => randomPut(provider, 'a/b/'))),
        Promise.all(Array.from({ length: 10 }).map(() => randomPut(provider, 'a/b/t/'))),
    ]);

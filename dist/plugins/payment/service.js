...
import { InvoiceStatus, SubscriptionPlan, SubscriptionPriceVariant, SubscriptionRecurring, SubscriptionStatus, } from './types';
const OnStripeEvent = (event, opts) => RawOnEvent(event, opts);
// Plan x Recurring make a stripe price lookup key
export function encodeLookupKey(plan, recurring, variant) {
    return `${plan}_${recurring}` + (variant ? `_${variant}` : '');
}
export function decodeLookupKey(key) {
...
let SubscriptionService = SubscriptionService_1 = class SubscriptionService {
    constructor(config, stripe, db, scheduleManager, event, features) {
        this.config = config;
        this.stripe = stripe;
        this.db = db;
        ...
        this.logger = new Logger(SubscriptionService_1.name);
    }
    async listPrices(user) {
        let canHaveEarlyAccessDiscount = false;
        let canHaveAIEarlyAccessDiscount = false;
        if (user) {
            canHaveEarlyAccessDiscount = await this.features.isEarlyAccessUser(user.id);
            canHaveAIEarlyAccessDiscount = await this.features.isEarlyAccessUser(user.id, EarlyAccessType.AI);
            const customer = await this.getOrCreateCustomer('list-price:' + randomUUID(), user);
        ...
        return list.data.filter(price => {
            ...
            if (plan === SubscriptionPlan.Pro) {
                return ((canHaveEarlyAccessDiscount && variant) ||
                    (!canHaveEarlyAccessDiscount && !variant));
            }
            if (plan === SubscriptionPlan.AI) {
                return ((canHaveAIEarlyAccessDiscount && variant) ||
                    (!canHaveAIEarlyAccessDiscount && !variant));
            }
            return false;
        });
    }
    async createCheckoutSession({ user, recurring, plan, promotionCode, redirectUrl, idempotencyKey, }) {
        ...
        if (currentSubscription) {
            throw new BadRequestException(`You've already subscribed to the ${plan} plan`);
        }
        const customer = await this.getOrCreateCustomer(`${idempotencyKey}-getOrCreateCustomer`, user);
        const { price, coupon } = await this.getAvailablePrice(customer, plan, recurring);
        let discounts = [];
        if (coupon) {
            discounts = [{ coupon }];
        }
        else if (promotionCode) {
            const code = await this.getAvailablePromotionCode(promotionCode, customer.stripeCustomerId);
            if (code) {
    ...
    async cancelSubscription(idempotencyKey, userId, plan) {
        ...
        if (subscriptionInDB.canceledAt) {
            throw new BadRequestException('Your subscription has already been canceled');
        }
        // should release the schedule first
        if (subscriptionInDB.stripeScheduleId) {
            const manager = await this.scheduleManager.fromSchedule(subscriptionInDB.stripeScheduleId);
            await manager.cancel(idempotencyKey);
            return this.saveSubscription(user, await this.stripe.subscriptions.retrieve(subscriptionInDB.stripeSubscriptionId));
        }
        else {
            // let customer contact support if they want to cancel immediately
            // see https://stripe.com/docs/billing/subscriptions/cancel
            const subscription = await this.stripe.subscriptions.update(subscriptionInDB.stripeSubscriptionId, { cancel_at_period_end: true }, { idempotencyKey });
            return await this.saveSubscription(user, subscription);
        }
    }
    async resumeCanceledSubscription(idempotencyKey, userId, plan) {
        const user = await this.db.user.findUnique({
            where: {
                id: userId,
            },
        ...
        if (subscriptionInDB.end < new Date()) {
            throw new BadRequestException('Your subscription is expired, please checkout again.');
        }
        if (subscriptionInDB.stripeScheduleId) {
            const manager = await this.scheduleManager.fromSchedule(subscriptionInDB.stripeScheduleId);
            ...
            const subscription = await this.stripe.subscriptions.update(subscriptionInDB.stripeSubscriptionId, { cancel_at_period_end: false }, { idempotencyKey });
            return await this.saveSubscription(user, subscription);
        }
    }
    async updateSubscriptionRecurring(idempotencyKey, userId, plan, recurring) {
        const user = await this.db.user.findUnique({
            where: {
                id: userId,
            },
        ...
        if (subscriptionInDB.recurring === recurring) {
            throw new BadRequestException(`You are already in ${recurring} recurring`);
        }
        const price = await this.getPrice(subscriptionInDB.plan, recurring);
        const manager = await this.scheduleManager.fromSubscription(`${idempotencyKey}-fromSubscription`, subscriptionInDB.stripeSubscriptionId);
    ...
    async onSubscriptionChanges(subscription) {
        subscription = await this.stripe.subscriptions.retrieve(subscription.id);
        if (subscription.status === 'active') {
            const user = await this.retrieveUserFromCustomer(typeof subscription.customer === 'string'
    ...
    async onInvoicePaid(stripeInvoice) {
    ...
    async saveInvoice(stripeInvoice) {
        stripeInvoice = await this.stripe.invoices.retrieve(stripeInvoice.id);
        if (!stripeInvoice.customer) {
            throw new Error('Unexpected invoice with no customer');
        }
        const user = await this.retrieveUserFromCustomer(typeof stripeInvoice.customer === 'string'
        ...
        if (stripeInvoice.attempt_count > 1) {
            const paymentIntent = await this.stripe.paymentIntents.retrieve(stripeInvoice.payment_intent);
    ...
    async saveSubscription(user, subscription) {
    ...
    async getOrCreateCustomer(idempotencyKey, user) {
        ...
        if (!customer) {
            ...
            if (stripeCustomersList.data.length) {
                ...
                stripeCustomer = await this.stripe.customers.create({ email: user.email }, { idempotencyKey });
            }
    ...
    async retrieveUserFromCustomer(customerId) {
        const customer = await this.db.userStripeCustomer.findUnique({
            where: {
        ...
        if (!user) {
            throw new Error(`Unexpected subscription created with unknown customer ${stripeCustomer.email}`);
        }
    ...
    async getPrice(plan, recurring, variant) {
        const prices = await this.stripe.prices.list({
            lookup_keys: [encodeLookupKey(plan, recurring, variant)],
        });
        if (!prices.data.length) {
            throw new BadRequestException(`Unknown subscription plan ${plan} with ${recurring} recurring`);
        }
     ...
     */
    async getAvailablePrice(customer, plan, recurring) {
        ...
        if (plan === SubscriptionPlan.Pro) {
            const canHaveEADiscount = isEaUser && !subscribed && recurring === SubscriptionRecurring.Yearly;
            const price = await this.getPrice(plan, recurring, canHaveEADiscount ? SubscriptionPriceVariant.EA : undefined);
            return {
                price,
            ...
            const isAIEaUser = await this.features.isEarlyAccessUser(customer.userId, EarlyAccessType.AI);
            const canHaveEADiscount = isAIEaUser && !subscribed && recurring === SubscriptionRecurring.Yearly;
            const price = await this.getPrice(plan, recurring, canHaveEADiscount ? SubscriptionPriceVariant.EA : undefined);
            return {
                price,
                coupon: !subscribed
    ...
    async getAvailablePromotionCode(userFacingPromotionCode, customer) {
    ...
    decodePlanFromSubscription(sub) {
...
], SubscriptionService.prototype, "onSubscriptionChanges", null);
__decorate([
    OnStripeEvent('customer.subscription.deleted'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], SubscriptionService.prototype, "onSubscriptionDeleted", null);
__decorate([
    OnStripeEvent('invoice.paid'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], SubscriptionService.prototype, "onInvoicePaid", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], SubscriptionService.prototype, "saveInvoice", null);
__decorate([

...
import { applyUpdate, Doc, encodeStateAsUpdate, encodeStateVector, transact, } from 'yjs';
import { Cache, CallTimer, Config, EventEmitter, mergeUpdatesInApplyWay as jwstMergeUpdates, metrics, OnEvent, } from '../../fundamentals';
...
export function isEmptyBuffer(buf) {
    return (buf.length === 0 ||
        // 0x0000
 ...
 */
let DocManager = DocManager_1 = class DocManager {
    constructor(db, config, cache, event) {
        this.db = db;
        this.config = config;
        this.cache = cache;
        this.event = event;
        this.logger = new Logger(DocManager_1.name);
        this.job = null;
        this.seqMap = new Map();
        this.busy = false;
    }
    ...
    async applyUpdates(guid, ...updates) {
        const doc = await this.recoverDoc(...updates);
        const useYocto = await this.config.runtime.fetch('doc/experimentalMergeWithYOcto');
        // test jwst codec
        if (useYocto) {
            ...
            try {
                ...
                if (!compare(yjsResult, jwstResult)) {
                    ...
                    this.logger.warn(`jwst codec result doesn't match yjs codec result for: ${guid}`);
                    log = true;
    ...
    async onSnapshotDeleted({ id, workspaceId, }) {
        await this.db.update.deleteMany({
            where: {
                id,
                workspaceId,
     ...
     */
    async push(workspaceId, guid, update, retryTimes = 10) {
        const timestamp = await new Promise((resolve, reject) => {
            defer(async () => {
                ...
                .subscribe({
                next: timestamp => {
                    this.logger.debug(`pushed 1 update for ${guid} in workspace ${workspaceId}`);
                    resolve(timestamp);
                },
                error: e => {
    ...
    async batchPush(workspaceId, guid, updates, retryTimes = 10) {
        const timestamp = await new Promise((resolve, reject) => {
            defer(async () => {
                ...
                .subscribe({
                next: timestamp => {
                    this.logger.debug(`pushed ${updates.length} updates for ${guid} in workspace ${workspaceId}`);
                    resolve(timestamp);
                },
                error: e => {
                    this.logger.error('Failed to push updates', e);
     ...
     */
    async getDocTimestamps(workspaceId, after = 0) {
     ...
     */
    async getBinary(workspaceId, guid) {
        const result = await this._get(workspaceId, guid);
        if (result) {
            if ('doc' in result) {
                return {
     ...
     */
    async getDocState(workspaceId, guid) {
     ...
     */
    async getSnapshot(workspaceId, guid) {
     ...
     * get pending updates
     */
    async getUpdates(workspaceId, guid) {
    ...
    async autoSquash() {
        ...
        await this.lockUpdatesForAutoSquash(workspaceId, id, async () => {
            try {
                ...
                this.logger.error(`Failed to apply updates for workspace: ${workspaceId}, guid: ${id}`);
                this.logger.error(e);
            }
        });
    }
    async getAutoSquashCandidate() {
     ...
     */
    async upsert(workspaceId, guid, doc, 
     ...
     */
    async squash(snapshot, updates) {
        if (!updates.length) {
            throw new Error('No updates to squash');
        }
        const last = updates[updates.length - 1];
        const { id, workspaceId } = last;
        const doc = await this.applyUpdates(id, snapshot ? snapshot.blob : Buffer.from([0, 0]), ...updates.map(u => u.blob));
        const done = await this.upsert(workspaceId, id, doc, last.createdAt, last.seq);
        if (done) {
            if (snapshot) {
            ...
            this.logger.debug(`Squashed ${updates.length} updates for ${id} in workspace ${workspaceId}`);
        }
    ...
    async getUpdateSeq(workspaceId, guid, batch = 1) {
        try {
    ...
    async updateCachedUpdatesCount(workspaceId, guid, count) {
        const result = await this.cache.mapIncrease(UPDATES_QUEUE_CACHE_KEY, `${workspaceId}::${guid}`, count);
        if (result <= 0) {
            await this.cache.mapDelete(UPDATES_QUEUE_CACHE_KEY, `${workspaceId}::${guid}`);
        }
    }
    async getAutoSquashCandidateFromCache() {
        const key = await this.cache.mapRandomKey(UPDATES_QUEUE_CACHE_KEY);
        if (key) {
            const cachedCount = await this.cache.mapIncrease(UPDATES_QUEUE_CACHE_KEY, key, 0);
            if (cachedCount > 0) {
    ...
    async doWithLock(lockScope, lockResource, job) {
    ...
    async lockUpdatesForAutoSquash(workspaceId, guid, job) {
        return this.doWithLock('doc:manager:updates', `${workspaceId}::${guid}`, job);
    }
...
], DocManager.prototype, "recoverDoc", null);
__decorate([
    OnEvent('workspace.deleted'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], DocManager.prototype, "onWorkspaceDeleted", null);
__decorate([
    OnEvent('snapshot.deleted'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], DocManager.prototype, "onSnapshotDeleted", null);
__decorate([
    CallTimer('doc', 'get_doc_timestamps'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", Promise)
], DocManager.prototype, "getDocTimestamps", null);
__decorate([
    CallTimer('doc', 'upsert'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, Doc,
        Date, Number]),
    __metadata("design:returntype", Promise)
], DocManager.prototype, "upsert", null);
__decorate([
    CallTimer('doc', 'squash'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Array]),
    __metadata("design:returntype", Promise)
], DocManager.prototype, "squash", null);
__decorate([
    Cron(CronExpression.EVERY_MINUTE),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], DocManager.prototype, "reportUpdatesQueueCount", null);
DocManager = DocManager_1 = __decorate([
    Injectable(),

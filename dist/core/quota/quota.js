import { formatDate, formatSize, QuotaSchema } from './types';
const QuotaCache = new Map();
export class QuotaConfig {
    static async get(tx, featureId) {
    ...
    constructor(data) {
        ...
        if (config.success) {
            ...
            throw new Error(`Invalid quota config: ${config.error.message}, ${JSON.stringify(data)})}`);
        }
    }
    get version() {
    ...
    get businessBlobLimit() {
        return (this.config.configs.businessBlobLimit || this.config.configs.blobLimit);
    }
    get storageQuota() {
